...
class LegendItem extends React.Component<any> {
        ...
        return (
            <svg style={{ float: "left" }} width={symbolWidth} height={symbolHeight}>
                <line
                    style={style}
                    x1={0}
                    y1={parseInt(symbolWidth / 2 as any, 10)}
                    x2={symbolWidth}
                    y2={parseInt(symbolWidth / 2 as any, 10)}
                    stroke="black"
                    strokeWidth="2"
                />
            </svg>
    ...
    renderDot(style) {
        const { symbolWidth, symbolHeight } = this.props;
        const w = parseInt(symbolWidth / 2 as any, 10);
        const h = parseInt(symbolHeight / 2 as any, 10);
        const radius = w * 0.75;
...
export default class Legend extends React.Component<InferProps<typeof Legend.propTypes>> {
     ...
     */
    providedStyle(category, type?): any {
        let style = {};
        if (this.props.style) {
