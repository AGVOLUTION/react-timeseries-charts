...
export default class EventHandler extends React.Component<EventHandlerProps, any> {
    ...
    handleScrollWheel(e) {
        ...
        let beginScaled = center - parseInt(((center - begin) * scale) as any, 10);
        let endScaled = center + parseInt(((end - center) * scale) as any, 10);

        // Duration constraint
        let duration = (end - begin) * scale;

        if (this.props.minDuration) {
            const minDuration = parseInt(this.props.minDuration as any, 10);
    ...
    handleMouseMove(e) {
        ...
        if (this.state.isPanning) {
            ...
            let newBegin = parseInt((this.state.initialPanBegin - timeOffset) as any, 10);
            let newEnd = parseInt((this.state.initialPanEnd - timeOffset) as any, 10);
            const duration = parseInt((this.state.initialPanEnd - this.state.initialPanBegin) as any, 10);
